...
El proyecto adopta la organización modular estricta exigida por la cátedra:
* `/datos`: Contiene el archivo fuente `dataset.csv` con los registros simulados de las ventas.
* `/scripts`: Aloja el código principal `analisis_datos.py` desarrollado en Python nativo.
* `/resultados`: Carpeta automatizada donde se exporta el reporte visual `grafico_ventas.png`.
* `.gitignore`: Configurado para excluir archivos temporales de Python y checkpoints de Jupyter/Colab.
...
3. `cantidad_vendida`: Volumen físico de unidades en la operación (entero).
